...
class PixiOverlay {
  ...
  private redraw(): void {
    ...
    // Position the Pixi.js canvas relative to the top-left of the map
    L.DomUtil.setPosition(this.app.canvas as HTMLCanvasElement, topLeft);
    ...
    this.container.children.forEach((node: PIXI.DisplayObject) => {
        ...
        // Adjust position of the Pixi.js object on the canvas
        sprite.position.set(pixiPoint.x  , pixiPoint.y );
  ...
  public addNode(lat: number, lng: number): void {
    ...
    // Adjust position of the Pixi.js object on the canvas
    node.position.set(pixiPoint.x - topLeft.x, pixiPoint.y - topLeft.y);
...
export default () => {
  ...
  canvas.artBoard.init().then(() => {
    const map = L.map(mapContainer, {zoomControl: false}).setView([ 17.45146258201848, 78.3812387672121], 5);
    ...
    pixiOverlay.addNode(17.45146258201848, 78.3812387672121 )
    pixiOverlay.addNode(9.5010343,76.3009063)
  });
